...
int	main(int argc, char **argv)
{
	...
	d.img.rx = X_REZ;
	d.img.ry = Y_REZ;
	d.map.un = (float)d.img.rx / ((float)(d.map.size_x + d.map.size_y) * (float)1.3);
	d.map.start.x = d.map.un * (d.map.size_x * (float)0.2);
	d.map.start.y = d.map.un * (d.map.size_y * (float)0.9);
	d.p.col = 0x00FF0000;
	window(&d);
	return (0);
}
