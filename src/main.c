...
/*   Updated: 2022/02/26 20:02:15 by dkocob        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	// d.map.start.x = round(d.map.size_x * d.map.un * cos(-30 * CONST));
	// d.map.start.y = round(d.map.size_x * d.map.un * sin(-30 * CONST));
	// d.map.start.x = round(d.map.un * cos(-30 * CONST));
	// d.map.start.y = round(d.map.un * sin(-30 * CONST));
	d.map.start.x = d.map.un * (d.map.size_x /4);
	d.map.start.y = d.map.un * (d.map.size_y * 2 );
	d.p.col = 0x00FF0000;
	window(&d);
	return (0);
}